...
class _DashboardState extends State<Dashboard> {
  ...
  Row buildTopBar(BuildContext context) {
    // Split the location display only if the newline character is present.
    List<String> locationParts = locationDisplay.contains('\n')
        ? locationDisplay.split('\n')
        : [locationDisplay, ''];  // Fallback to prevent RangeError
    return Row(
        ...
        Row(
          ...
          _getUserLocation();
            ...
            Column(
              ...
              children: [
                // Check if the area and locality are available.
                Text(
                  locationParts[0],  // Area and Locality
                  style: TextStyle(
                    fontSize: 18,
                ...
                if (locationParts.length > 1)  // Only display country if it's available
                  Text(
                    locationParts[1],  // Country
                    style: TextStyle(
                      fontSize: 16,
  ...
  Future<void> _getUserLocation() async {
    ...
    if (_userLocation != null) {
      ...
      setState(() {
        // Here, I am assuming 'subAdministrativeArea' will give you the area like 'Mhalgi Nagar'
        // and 'locality' will give you the city like 'Nagpur'. Adjust the fields based on actual API responses.
        String area = place.subLocality ?? 'Unknown Area';
        locality = place.locality ?? 'Unknown City';
        country = place.country ?? 'Unknown Country';
        locationDisplay = "$area, $locality\n$country";
      });
    }
  }
